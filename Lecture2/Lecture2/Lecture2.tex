...
\author{Pingbang Hu}
\newcommand{\Title}{Honor Mathematics Lecture 2}
\newcommand{\SubTitle}{\textbf{From Numbers to Sequences}\\``We define everything as they now defined for a good reason..."}
...
\begin{frame}
    ...
    \end{equation*}
    We will have the following properties we simply take it as \emph{axioms}.
    \begin{table}
        \centering
        \resizebox{11cm}{!}{%
            \begin{tabular}{ccc}
...
\begin{frame}
    ...
    \begin{enumerate}
        ...
        \item $-a\in P$
    \end{enumerate}
    which is so-called \textbf{trichotomy law}(P10).)\\
    Furthermore, we assume that the set of positive number $P$ is closed under addition and multiplication.(P11, P12)\\
\end{frame}

\begin{frame}
    \frametitle{Real Numbers}
    We will adapt all axioms we had from rational numbers, except the last three axioms
    regard to trichotomy law. By replacing $P$ for $\mathbb{R}$, we \textbf{almost}
    define our set of real numbers $\mathbb{R}$\\
    \vspace{1em}
    The last thing we need for $\mathbb{R}$ is listed below(P13):\\
    \vspace{1em}
    If $A\subset \mathbb{R}$, $A\neq \emptyset$ is bounded above, then there exists a least upper
    bound for $A$ in $\mathbb{R}$.\\
    \vspace{1em}
    (Why? And why we want to define real numbers?)
\end{frame}

\begin{frame}
    \frametitle{Real Numbers}
    It is important for us to study whether a set is bounded or not. More importantly, we want to give the \textbf{tightest} bounded if possible. Hence, we have
    \begin{itemize}
        \item lower bound
        \item upper bound
        \item minimum
        ...
        \item infimum(greatest lower bound)
    \end{itemize}
    for a set.

    \vspace{1em}
    Question. Why is P13 sufficient to guarantee the existence of greatest lower bound?
\end{frame}

\begin{frame}
    \frametitle{Complex Numbers}
    You just need to know how to perform basic complex numbers' computation and some basic properties.
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Usually we \emph{assume} the total set is $\mathbb{R}$
